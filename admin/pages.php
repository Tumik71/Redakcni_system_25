...
$pdo = Database::conn();
$rows = $pdo->query('SELECT id, title, slug, created_at FROM pages ORDER BY created_at DESC')->fetchAll();
?>
<!doctype html>
<html lang="cs">
<head>
...
<body class="bg-white dark:bg-slate-900 font-sans font-light">
  <nav class="fixed top-0 inset-x-0 z-30 backdrop-blur bg-white/70 dark:bg-slate-900/70 border-b border-slate-200 dark:border-slate-700">
    <div class="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
      <div class="font-semibold tracking-tight">Stránky</div>
  ...
  <div class="max-w-6xl mx-auto p-6 pt-20">
    <div class="flex justify-between mb-4">
      <h1 class="text-xl font-semibold">Stránky</h1>
      <a href="/admin/page_edit.php" class="bg-brand text-white px-3 py-2 rounded">Nová stránka</a>
    </div>
    <table class="w-full bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
      <thead><tr class="text-left">
        <th class="p-3">Název</th><th class="p-3">Slug</th><th class="p-3">Akce</th>
      </tr></thead>
      <tbody>
        <?php foreach ($rows as $r): ?>
          <tr class="border-t border-slate-200 dark:border-slate-700">
            ...
            <td class="p-3 space-x-2">
              <a class="text-brand" href="/admin/page_edit.php?id=<?= $r['id'] ?>">Upravit</a>
              <a class="text-red-500" href="/admin/page_delete.php?id=<?= $r['id'] ?>" onclick="return confirm('Opravdu smazat?')">Smazat</a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
