...
<head>
  ...
  <script>tailwind.config={darkMode:'class',theme:{extend:{colors:{brand:{DEFAULT:'#0ea5e9',dark:'#0369a1'},ink:'#0f172a'},fontFamily:{sans:['Inter','ui-sans-serif','system-ui']}}}}</script>
...
<body class="bg-white dark:bg-slate-900 text-ink dark:text-slate-100 font-sans font-light">
  ...
  <header class="relative pt-24">
    ...
    <div class="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-8 items-center">
      <div class="space-y-4">
        ...
        <p class="text-lg text-gray-600 dark:text-slate-300">Instalátor, administrace, API, média a CI/CD. Vše připraveno pro snadné nasazení na ISPConfig.</p>
  ...
  <script>
    function syncIcons(){var d=document.documentElement.classList.contains('dark');var sun=document.getElementById('iconSun');var moon=document.getElementById('iconMoon');if(sun&&moon){sun.classList.toggle('hidden', d);moon.classList.toggle('hidden', !d);}}
    syncIcons();
    var b=document.getElementById('themeToggle');if(b){['click','touchstart','pointerdown','keydown'].forEach(function(ev){b.addEventListener(ev,function(e){if(ev==='keydown'){if(e.key!=='Enter'&&e.key!==' ')return;}e.preventDefault();var d=!document.documentElement.classList.contains('dark');document.documentElement.classList.toggle('dark', d);try{localStorage.setItem('theme',d?'dark':'light');}catch(e){}syncIcons();},{passive:false});});}
  </script>
</body>
</html>
