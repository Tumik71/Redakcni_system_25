...
class Theme {
    public static function injectHead(): void {
        ...
        echo $css.$js.'<script>(function(){function T(){var d=!document.documentElement.classList.contains("dark");document.documentElement.classList.toggle("dark",d);try{localStorage.setItem("theme",d?"dark":"light");}catch(_){ }var s=document.getElementById("iconSun"),m=document.getElementById("iconMoon");if(s&&m){s.classList.toggle("hidden",d);m.classList.toggle("hidden",!d);}}function onEvt(e){var el=e.target&&e.target.closest?e.target.closest("[data-theme-toggle], #themeToggle"):null;if(!el)return;T();}['click','touchstart','touchend','pointerdown','pointerup'].forEach(function(ev){document.addEventListener(ev,onEvt,{passive:true});});document.addEventListener("keydown",function(e){var el=e.target&&e.target.closest?e.target.closest("[data-theme-toggle], #themeToggle"):null;if(!el)return;if(e.key==="Enter"||e.key===" "){e.preventDefault();T();}});} )();</script>\n';
    }
}
