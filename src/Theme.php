...
class Theme {
    public static function injectHead(): void {
        ...
        echo $css.$js.'<script>(function(){document.addEventListener("click",function(e){var el=e.target.closest("[data-theme-toggle], #themeToggle");if(!el)return;var d=!document.documentElement.classList.contains("dark");document.documentElement.classList.toggle("dark",d);try{localStorage.setItem("theme",d?"dark":"light");}catch(_){ }var s=document.getElementById("iconSun");var m=document.getElementById("iconMoon");if(s&&m){s.classList.toggle("hidden",d);m.classList.toggle("hidden",!d);}});document.addEventListener("keydown",function(e){var el=e.target.closest("[data-theme-toggle], #themeToggle");if(!el)return;if(e.key==="Enter"||e.key===" "){e.preventDefault();var d=!document.documentElement.classList.contains("dark");document.documentElement.classList.toggle("dark",d);try{localStorage.setItem("theme",d?"dark":"light");}catch(_){ }var s=document.getElementById("iconSun");var m=document.getElementById("iconMoon");if(s&&m){s.classList.toggle("hidden",d);m.classList.toggle("hidden",!d);}}});})();</script>\n';
    }
}
